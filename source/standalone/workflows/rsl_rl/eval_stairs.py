...
def main():
    ...
    env_cfg.scene.terrain.terrain_generator.num_rows = 15
    env_cfg.scene.terrain.terrain_generator.num_cols = 15
    env_cfg.scene.terrain.terrain_generator.curriculum = True
    env_cfg.scene.terrain.terrain_generator.difficulty_range = (0.0, 1.0)
    env_cfg.commands.base_velocity.rel_standing_envs = 0.0
    env_cfg.commands.base_velocity.ranges.lin_vel_y = [0.4, 0.5]
    ...
    # reset environment
    env.unwrapped.scene.terrain.terrain_levels = torch.randint_like(
        env.unwrapped.scene.terrain.terrain_levels,
        env.unwrapped.scene.terrain.max_terrain_level,
    )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            #
            rows = env.unwrapped.scene.terrain.terrain_levels
            cols = env.unwrapped.scene.terrain.terrain_types
            cache["step_height"] = env.unwrapped.scene.terrain.terrain_params[
                "step_height"
            ][rows, cols].clone()
            cache["step_width"] = env.unwrapped.scene.terrain.terrain_params[
                "step_width"
            ][rows, cols].clone()
            cache["num_steps"] = env.unwrapped.scene.terrain.terrain_params[
                "num_steps"
            ][rows, cols].clone()
            cache["root_pos_w"] = env.unwrapped.scene.articulations[
                "robot"
            ].data.root_pos_w.clone()
            cache["env_origins"] = env.unwrapped.scene.env_origins.clone()
            ...
            if dones.any():
                done_indices = torch.nonzero(dones).squeeze(dim=1)
                distance = (
                    cache["root_pos_w"][:, 1]
                    - cache["env_origins"][:, 1]
                    ...
                    - (cache["num_steps"] + 1) * cache["step_width"]
                )[done_indices]
                success = distance >= 0
                ...
                for i in range(success.shape[0]):
                    print(
                        "h=",
                        cache["step_height"][done_indices][i].item(),
                        "w=",
                        cache["step_width"][done_indices][i].item(),
                        "d=",
                        distance[i].item(),
                    )
                    success_info.append(
                        {
                            "num_steps": cache["num_steps"][done_indices][i].item(),
                            "width": cache["step_width"][done_indices][i].item(),
                            "height": cache["step_height"][done_indices][i].item(),
                            "success": success[i].item(),
                            "cmd": env.unwrapped.command_manager.get_command(
                                "base_velocity"
                            )[i].tolist(),
                            "final_pos": cache["root_pos_w"][done_indices][i].tolist(),
                            "env_origin": cache["env_origins"][done_indices][
                                i
                            ].tolist(),
                            "y_coordinate_origin_relative_to_first_stair_step": env.cfg.scene.terrain.terrain_generator.sub_terrains[
                                "stairs"
                            ].y_coordinate_origin_relative_to_first_stair_step,
                        }
                    )

                # Random environment origin / subterrain
                env.unwrapped.scene.terrain.terrain_levels[done_indices] = (
                    torch.randint_like(
                        env.unwrapped.scene.terrain.terrain_levels[done_indices],
                        env.unwrapped.scene.terrain.max_terrain_level,
                    )
                )
                env.unwrapped.scene.terrain.terrain_types[done_indices] = (
                    torch.randint_like(
                        env.unwrapped.scene.terrain.terrain_types[done_indices],
                        env.unwrapped.scene.terrain.terrain_origins.shape[1],
                    )
                )
                obs, _ = env.get_observations()
    ...
    if args_cli.evaluate:
        ...
        with open(os.path.join(eval_metric_folder, "success.yaml"), "w") as f:
            yaml.dump(success_info, f)
